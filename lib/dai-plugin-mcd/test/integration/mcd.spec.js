...
beforeAll(async () => {
  ...
  const network = process.env.NETWORK === 'test' ? 'testnet' : process.env.NETWORK;
...
describe.each(scenarios)('%s', (ilk, GEM) => {
  ...
  test('openLockAndDraw, get, draw, wipe, wipeAndFree', async () => {
    ...
    startingDaiBalance = await dai.balance();
    await dai.approveUnlimited(proxy);
    const cdp = await mgr.openLockAndDraw(ilk, GEM(.01), MDAI(.1));
    await expectValues(cdp, {
      collateral: .01,
      debt: .1,
      myDai: startingDaiBalance.plus(.1),
      myGem: startingGemBalance.minus(.01)
    });

    const sameCdp = await mgr.getCdp(cdp.id);
    await expectValues(sameCdp, { collateral: .01, debt: .1 });
    ...
    await expectValues(cdp, { debt: .15, myDai: startingDaiBalance.plus(.15) });

    await cdp.wipeAndFree(MDAI(.1), GEM(0.005));
    await expectValues(cdp, {
      collateral: 0.005,
      debt: 0.05,
