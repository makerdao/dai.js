...
async function expectUtilValues(cdp, { val, ratio, isSafe, min, avail, dai }) {
  const [
    debtValue,
    collateralValue,
    collateralAmount,
  ...
  const minVal = debtValue.div(MDAI(val)).toNumber();

  if (val !== undefined) {
    expect(collateralValue).toEqual(val);
  }
  if (ratio !== undefined) {
    expect(collateralizationRatio).toEqual(ratio);
  }
  if (safe !== undefined) {
    expect(safe).toBe(isSafe);
  }
  if (min !== undefined) {
    expect(minCollateral.toNumber()).toEqual(minVal);
  }
  if (avail !== undefined) {
    expect(availCollateral.toNumber()).toBe(
      collateralAmount.toNumber() - minVal
    );
  }
  if (dai !== undefined) {
...
describe.each([
  ...
  test('openLockAndDraw, get, draw, wipe, wipeAndFree', async () => {
    ...
    const price = await cdp._cdpType.getPrice();
