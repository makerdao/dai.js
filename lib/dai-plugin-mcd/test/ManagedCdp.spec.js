...
import { DGX, REP, USD_REP, MDAI } from '../src';
...
describe.each([
  [
    ...
    async () => {
      ...
      await setDebtCeiling(maker, MDAI(50), ETH);
    }
  ],
  [
    'REP',
    ...
    async () => {
      ...
      await maker.getToken('REP').approveUnlimited(proxy);
      await setPrice(maker, USD_REP(100));
      await setDebtCeiling(maker, MDAI(50), REP);
    }
  ]
])('%s', (symbol, GEM, setup) => {
  ...
  test('openLockAndDraw, get, draw, wipe, wipeAndFree', async () => {
    ...
    const cdp = await mgr.openLockAndDraw(GEM(1), MDAI(1));
    await expectValues(cdp, {
      collateral: 1,
      debt: 1,
    ...
    const sameCdp = mgr.getCdp(cdp.id, GEM);
...
describe('without proxy', () => {
  ...
  test('create proxy and open', async () => {
    const cdp = await mgr.openLockAndDraw(ETH(2));
    expect(cdp.id).toBeGreaterThan(0);
    cdpId = cdp.id;
  });

  test("prevent access to someone else's CDP", async () => {
    maker.useAccount('default');
    ...
    try {
      ...
      expect(err.message).toMatch(/wipeAndFreeETH.*reverted/);
    }
  });
});
