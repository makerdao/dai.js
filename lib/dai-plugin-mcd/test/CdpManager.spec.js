...
test('open & lock ETH', async () => {
  ...
  const locked = await cdp.getCollateralValue();
  expect(locked).toEqual(ETH(1));

  const drawn = await cdp.getDebtValue();
  expect(drawn).toEqual(DAI(0));
});
...
test.skip('open, lock ETH, draw Dai', async () => {
  ...
  expect(cdp.id).toBeGreaterThan(0);
  const locked = await cdp.getCollateralValue();
  expect(locked).toEqual(REP(1));

  const drawn = await cdp.getDebtValue();
  expect(drawn).toEqual(DAI(1));
});
