...
test('getCdps gets all CDPs from a proxy when there are no CDPs', async () => {
  const currentProxy = await maker.currentProxy();
  const cdps = await cdpMgr.getCdps(currentProxy);

  expect(cdps.length).toEqual(0);
});

test('getCdps gets all CDPs from the proxy', async () => {
