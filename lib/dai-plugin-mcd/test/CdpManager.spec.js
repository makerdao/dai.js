...
import { dummyEventData, formattedDummyEventData } from './ManagedCdp.spec.js';
...
test('getCombinedEventHistory', async () => {
  ...
  maker.service(ServiceRoles.QUERY_API).getCdpEventsForArrayOfIlksAndUrns = mockFn;
  const events = await cdpMgr.getCombinedEventHistory(proxy);
  expect(mockFn).toBeCalled();
  const currency = maker.service(ServiceRoles.CDP_TYPE).getCdpType(null, events[0].ilk).currency;
