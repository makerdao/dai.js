...
test('getCdps gets all CDPs from current proxy when there are no CDPs', async () => {
  const cdps = await cdpMgr.getCdps();

  expect(cdps.length).toEqual(0);
});

test('getCdps gets all CDPs from the current proxy', async () => {
  const cdp1 = await cdpMgr.open('ETH-A');
  const cdp2 = await cdpMgr.open('ETH-B');

  const cdps = await cdpMgr.getCdps();

  const gottenCdp1 = cdps.find(cdp => cdp.id === cdp1.id);
  const gottenCdp2 = cdps.find(cdp => cdp.id === cdp2.id);

  expect(gottenCdp1.ilk).toEqual(cdp1.ilk);
  expect(gottenCdp2.ilk).toEqual(cdp2.ilk);
});

test('getCdps returns empty when there is no proxy', async () => {
