...
test('get ilk id', async () => {
  ...
  expect(ilkId).toBe('0x455448');
});

test('get collateral symbol', async () => {
  const ilkSymbol = await ethCdpType.collateralSymbol;
  expect(ilkSymbol).toBe('ETH');
});
...
test.skip('list all cdp type objects', async () => {
  const allCdpTypes = service.listAllCdpTypes();
  console.log('allCdpTypes', allCdpTypes);
});
