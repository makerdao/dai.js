...
export async function mcdMaker({
  ...
  const maker = await Maker.create(preset, {
    plugins: [
      [McdPlugin, { addressOverrides, network, simplePriceFeeds: true }]
    ],
    log: false,
    ...settings
  });
...
async function execPlan(maker, data) {
  ...
  const planAddress = scs.getContractAddress('MCD_PAUSE_PLAN');

  const era = Math.ceil(new Date().getTime() / 1000 + 1);
  await pause.plan(planAddress, data, era);
...
export async function setDebtCeiling(maker, amount, ilk) {
  ...
  const plan = scs.getContract('MCD_PAUSE_PLAN');
