...
test.only('getPriceHistoryForPip for ETH', async () => {
  ...
  expect(!!prices[1].val && !!prices[1].blockNumber).toBe(true);
...
test('getCdpEventsForIlkAndUrn', async () => {
  ...
  expectCdpEventObject(events[1], ETH);
  expect(events[0].time > events[1].time).toBe(true);
});
