...
export default class CdpType {
  ...
  getTotalDebtSync() {
    assert(this.cache.vatInfo, 'vatInfo is not cached');
    const { Art, rate } = this.cache.vatInfo;
    return MDAI.wei(Art)
      .times(rate)
      .shiftedBy(-27);
  }
  ...
  getDebtCeilingSync() {
    assert(this.cache.vatInfo, 'vatInfo is not cached');
    return MDAI.rad(this.cache.vatInfo.line);
  }
  ...
  getLiquidationRatioSync() {
    assert(this.cache.spotInfo, 'spotInfo is not cached');
    const { mat } = this.cache.spotInfo;
  ...
  getPriceSync() {
    const { vatInfo, par } = this.cache;
    assert(vatInfo && par, 'vatInfo or par is not cached');
    const parBN = new BigNumber(par.toString()).dividedBy(RAY);
    const spot = new BigNumber(vatInfo.spot.toString()).dividedBy(RAY);
    const ratio = createCurrencyRatio(USD, this.currency);
    const price = spot
      .times(parBN)
      .times(this.getLiquidationRatioSync().toNumber());
    return ratio(price);
  }
  ...
  getLiquidationPenaltySync() {
    assert(this.cache.catInfo, 'catInfo is not cached');
    const { chop } = this.cache.catInfo;
  ...
  getAnnualStabilityFeeSync() {
    assert(this.cache.jugInfo, 'jugInfo is not cached');
    const { duty } = this.cache.jugInfo;
