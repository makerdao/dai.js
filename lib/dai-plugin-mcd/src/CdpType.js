...
export default class CdpType {
  ...
  async getPriceHistory(num = 100){
    const prices = this._cdpTypeService.get(ServiceRoles.QUERY_API).getPriceHistoryForPip(this._pipAddress, this.currency, num);
    return Promise.all(
      prices.map( async e => {
        let resultObj = {};
        resultObj.price = this.currency.wei(e.val);
        resultObj.time = await this._blockNumberToDateTime(e.blockNumber); //this will be updated to grab date directly from vulcanize
        return resultObj;
      })
    );
  }
