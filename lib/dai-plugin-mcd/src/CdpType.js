...
export default class CdpType {
  ...
  async getTotalDebt() {
    const { Art, rate } = await this._getVat();
    return MDAI.wei(Art)
      .times(rate)
      .shiftedBy(-27);
  }
  ...
  async _getPar() {
    if (!this._par) this._par = await this._systemData.spot.par();
    return this._par;
  }

  async _getVat() {
    if (!this._vat) this._vat = await this.ilkInfo();
    return this._vat;
  }

  async _getCat() {
    if (!this._cat) this._cat = await this.ilkInfo('cat');
    return this._cat;
  }

  async _getSpot() {
    if (!this._spot) this._spot = await this.ilkInfo('spot');
    return this._spot;
  }

  async _getJug() {
    if (!this._jug) this._jug = await this.ilkInfo('jug');
    return this._jug;
  }
