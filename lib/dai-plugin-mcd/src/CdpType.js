...
import { MDAI, USD, ETH } from './index';
...
export default class CdpType {
  ...
  async getTotalCollateral(unit = this.currency) {
    const adapterAddress = this._systemData
      .get('smartContract')
      .getContractAddress('MCD_JOIN_' + this.ilk);
    let { symbol } = this.currency;

    // we handle the ETH ilk differently because its adapter is a GemJoin which
    // converts the ETH to WETH, so we need to check the WETH balance
    if (this.currency === ETH) symbol = 'WETH';
