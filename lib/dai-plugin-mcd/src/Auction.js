...
import { getIlkForCurrency, getFlipContractNameForCurrency } from './utils';

export default class Auction {
  constructor(smartContractService, currency) {
    if (currency.symbol==='DAI') this._auctionContract = smartContractService.getContract('MCD_FLAP'); //surplus auction is auctioning off Dai
    else if(currency.symbol==='MKR') this._auctionContract = smartContractService.getContract('MCD_FLOP'); //debt auction is auctioning off Mkr
    else this._auctionContract = smartContractService.getContract(getFlipContractNameForCurrency(currency));
  }
