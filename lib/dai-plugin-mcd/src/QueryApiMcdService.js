...
export default class QueryApi extends PublicService {
  ...
  async getCdpEventsForIlkAndUrn(ilk, urn, currency, ilkNameReplacement = null) {
    ...
    return Promise.all(
      data.urnFrobs.nodes.map(async e => {
        ...
        returnObj.time = await this._blockNumberToDateTime(e.tx.blockHeight);
  ...
  async getPriceHistoryForPip(pipAddress, currency, num = 500) {
