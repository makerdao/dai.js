...
export default {
  addConfig: (
    _,
    { cdpTypes = defaultCdpTypes, network = 'mainnet', addressOverrides } = {}
  ) => {
    if (addressOverrides) {
      addContracts = mapValues(addContracts, (contractDetails, name) => ({
        ...contractDetails,
        address: addressOverrides[name] || contractDetails.address[network] //double check this works later
      }));
    }
    ...
    //remove contracts for which we don't have addresses for the given network
