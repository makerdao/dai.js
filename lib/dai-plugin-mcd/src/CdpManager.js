  ...
  async getCombinedEventHistory(proxyAddress) {
    ...
    const ilksAndUrns = await Promise.all(
      cdpIds.map(async c => {
        ...
        returnObj.ilkName = stringToBytes(c.ilk);
        return returnObj;
      })
    );
    console.log('ilksAndUrns', ilksAndUrns);
    //const events = await this.get(QUERY_API).getCdpEventsForArrayOfIlksAndUrns(cdpIds);
