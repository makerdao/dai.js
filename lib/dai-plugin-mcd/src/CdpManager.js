...
import { parseFrobEvents } from './ManagedCdp';
...
export default class CdpManager extends LocalService {
  ...
  async getCombinedEventHistory(proxyAddress) {
    ...
    const ilksAndUrns = await Promise.all(
      cdpIds.map(async c => {
        const urn = await this.getUrn(c.id); //will add caching to this function
    ...
    return parseFrobEvents(events, this.get(CDP_TYPE));
  }
