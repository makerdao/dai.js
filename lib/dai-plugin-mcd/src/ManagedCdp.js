...
export default class ManagedCdp {
  ...
  async getCollateralValue() {
    ...
    return ratio(collateral.times(price));
  }
  ...
  async getCollateralizationRatio() {
    ...
    return collateral.div(debt).toNumber();
  }
  ...
  async daiAvailable() {
    ...
    const dai = USD(collateral.toNumber())
      .div(USD(liquidationRatio))
      .minus(USD(debt.toNumber()));
    return dai.gte(0) ? MDAI(dai) : MDAI(0);
  }
