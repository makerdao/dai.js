...
export default class ManagedCdp {
  ...
  getCollateralAmountSync() {
    assert(this.cache.urnInfo, 'urnInfo is not cached');
    return this.currency.wei(this.cache.urnInfo.ink);
  }
  ...
  getCollateralValueSync() {
    return this.getCollateralAmountSync().times(this.type.getPriceSync());
  }
  ...
  getDebtValueSync() {
    assert(
      this.cache.urnInfo && this.type.cache.vatInfo,
      'urnInfo or vatInfo is not cached'
    );
    const art = MDAI.wei(this.cache.urnInfo.art);
    return art.times(this.type.cache.vatInfo.rate).shiftedBy(-27);
  }
  ...
  getCollateralizationRatioSync() {
    const debt = this.getDebtValueSync();
    if (debt.eq(0)) return Infinity;
    return this.getCollateralValueSync().div(debt);
  }
  ...
  getLiquidationPriceSync() {
    const collateral = this.getCollateralAmountSync();
    if (collateral.eq(0)) {
      const ratio = createCurrencyRatio(USD, this.currency);
      return ratio(Infinity);
    }
    return this.getDebtValueSync()
      .times(this.type.getLiquidationRatioSync())
      .div(collateral);
  }
  ...
  getDaiAvailableSync() {
    const dai = this.getCollateralValueSync()
      .div(this.type.getLiquidationRatioSync())
      .minus(this.getDebtValueSync());
