...
export default class ManagedCdp {
  ...
  async getCollateralizationRatio() {
    ...
    return (collateral.times(price).div(debt).toNumber() * 100).toFixed(2)
  }
  ...
  async getLiquidationPrice() {
    ...
    return (debt.times(liquidationRatio).div(collateral).toNumber()).toFixed(2)
  ...
  async daiAvailable() {
    ...
    return (collateral.times(price).toNumber() / liquidationRatio - debt.toNumber()).toFixed(2);
  }
