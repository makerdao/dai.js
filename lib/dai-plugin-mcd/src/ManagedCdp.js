...
export default class ManagedCdp {
  ...
  async minCollateral() {
    ...
    const lockedCollateral = debt.times(liquidationRatio).div(price);
    return lockedCollateral.gt(collateral) ? collateral : lockedCollateral;
  }
  ...
  async _getUrnInfo() {
    if (!this._urnInfo)
      this._urnInfo = await this._cdpManager.vat.urns(
        stringToBytes(this.ilk),
        this._cdpManager.getUrn(this.id)
      );
    return this._urnInfo;
  }
  ...
  async _getIlkInfo() {
    if (!this._ilkInfo) {
      ...
      this._ilkInfo = await _cdpType.ilkInfo();
    }
    return this._ilkInfo;
  }
