...
export default class ManagedCdp {
  ...
  async getCollateralizationRatio() {
    ...
    assert(debt.gte(0), 'Debt cannot be zero');
  ...
  async getLiquidationPrice() {
    ...
    assert(collateral.gte(0), 'Collateral cannot be zero');
  ...
  async getMinCollateralAmount() {
    ...
    const lockedCollateral = debt
      .times(liquidationRatio)
      .div(MDAI(price.toNumber()));
  ...
  async getCollateralAvailable() {
    ...
    return collateral.minus(ETH(lockedCollateral.toNumber()));
  }
