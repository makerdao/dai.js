...
export function parseFrobEvents(events, cdpTypeService) {
  return events.map(e => {
    let returnObj = {};
    const ilk = bytesToString(e.ilkName);
    const currency = cdpTypeService.getCdpType(null, ilk).currency;
    ...
    if (parseInt(e.dink) !== 0) {
      //could use .isEqual on the currency object instead?
...
export default class ManagedCdp {
  ...
  async getEventHistory() {
    ...
    return parseFrobEvents(events, this._cdpManager.get(ServiceRoles.CDP_TYPE));
  }
