import {default as provider, TestAccountProvider} from '../../src/utils/TestAccountProvider';

test('should reject illegal indices', () => {
  const msg = 'Index must be a natural number between 0 and ' + (provider._accounts.addresses.length - 1);
...
test('should reject accounts objects', () => {
  ...
  expect(() => new TestAccountProvider({ addresses: ['x', 'y', 'z'], keys: ['x', 'y']}))
...
test('test provider provides 1000 addresses and keys', () => {
  ...
  expect(error).toBe(-1);
  expect(() => provider.nextAccount()).toThrow('No more test accounts available.');
