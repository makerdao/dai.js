import ServiceManager, { UnknownDependencyError, DependencyNotResolvedError } from './ServiceManager';

const name = 'IAmAService', _ = ()=>{};

test("should only mark objects with a method manager() returning a ServiceManager as valid", () => {
  expect(ServiceManager.isValidService({ manager: () => new ServiceManager(name) })).toBe(true);
...
test("should expose the provided dependencies through dependencies()", () => {
  expect(new ServiceManager(name, [name, name]).dependencies()).toEqual([name, name]);
});
...
test("should throw when injecting an unknown dependency", () => {
  expect(() => new ServiceManager(name, ['ValidDependency']).inject('InvalidDependency', {}))
    .toThrow(UnknownDependencyError.Error);
});

test("should throw when injecting an invalid service as dependency", () => {
  expect(() => new ServiceManager(name, ['ValidDependency']).inject('ValidDependency', { manager: 'NotAServiceManager' }))
...
test("should return an injected dependency by name", () => {
  ...
  expect(new ServiceManager(name, ['ServiceA']).inject('ServiceA', serviceA).dependency('ServiceA'))
    .toBe(serviceA);
});
...
test("should initialize, connect, or authenticate its dependencies before itself", (done) => {
  const log = [],
    dep = new ServiceManager('dep', [], () => log.push('DI'), () => log.push('DC'), () => log.push('DA')),
    svc = new ServiceManager('svc', ['dep'], () => log.push('SI'), () => log.push('SC'), () => log.push('SA'));
...
test("should build and return a valid service object when calling createService()", () => {
  expect(ServiceManager.isValidService(new ServiceManager(name, ['ServiceA']).createService())).toBe(true);
});
