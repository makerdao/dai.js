  ...
  run() {
    try {
      ...
      return parentContract[this._method].apply(parentContract[this._method], this._args)
        .then(value => [
          new MethodNode(this._method, this._args, this._contract, this.id(), value),
          []
        ])
        .catch(reason => [
          new MethodNode(this._method, this._args, this._contract, this.id(), reason, true),
          []
        ]);

    } catch(error) {
      return Promise.resolve([
        new MethodNode(this._method, this._args, this._contract, this.id(), error, true),
        []
      ]);
    }
  }
