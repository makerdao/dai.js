...
test.skip('should successfully join and exit PETH', async () => {
...
test.skip('should return the wrapper ratio', async () => {
  const ratio = await peth.wrapperRatio();
  expect(ratio).toEqual(WETH(1));
});

test.skip('should return the join price in weth', async () => {
  const price = await peth.joinPrice(3);
  expect(price).toEqual(WETH(3));
});

test.skip('should return the exit price in weth', async () => {
